...
package com.accenture.im.logicserver;
...
import com.accenture.im.proxy.NetMsgHeader;
import com.accenture.im.repository.ChatRoomRepository;
import com.accenture.im.sample.chat.proto.Messagepush;
import com.accenture.im.utils.BaseConstants;
import com.accenture.im.utils.MyBatisSqlSessionFactory;
...
public class TopicChats {
    ...
    private ChatRoomRepository chatRoomRepository = MyBatisSqlSessionFactory.getSqlSession().getMapper(ChatRoomRepository.class);
    ...
    private TopicChats() {
    	List<ChatRoomEntity> chats = chatRoomRepository.selectByLoginName("im");
        chats.forEach(it -> {
            ConcurrentLinkedDeque<ChannelHandlerContext> ctxs = new ConcurrentLinkedDeque<>();
            topicJoiners.put(it.getChatRoomName(), ctxs);
        });
